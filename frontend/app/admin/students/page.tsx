    ...
    <AdminLayout>
      <div className="max-w-7xl mx-auto space-y-6 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage registered students, view details, and control access.</p>
        </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {paginatedStudents.map(student => (
                      <tr key={student.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                            student.is_active ? 'bg-[var(--color-success-50)] text-[var(--color-success)]' : 'bg-red-100 text-red-700'
                          }`}>
                ...
                {paginatedStudents.map(student => (
                  <div key={student.id} className="p-4 flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${student.is_active ? 'bg-[var(--color-success-50)] text-[var(--color-success)]' : 'bg-red-100 text-red-700'}`}>
                        {student.is_active ? 'Active' : 'Inactive'}
                      </span>
                    </div>
            ...
            <div className="flex items-center gap-4 pb-6 border-b border-slate-100">
              ...
              <div>
                ...
                <div className="flex gap-2 mt-2">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${selectedStudent.is_active ? 'bg-[var(--color-success-50)] text-[var(--color-success)]' : 'bg-red-100 text-red-700'}`}>
