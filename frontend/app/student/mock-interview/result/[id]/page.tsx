...
export default function MockInterviewResultPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function loadResult() {
      ...
      try {
        ...
        if (sessionData.student_id !== user.id) {
          setError('You do not have permission to view these interview results.');
          return;
        }
      ...
      <StudentLayout>
        <div className="flex items-center justify-center h-96">
          <div className="text-center space-y-3">
            ...
            <p className="text-xs font-semibold text-[var(--color-text-secondary)]">Loading performance evaluation...</p>
          </div>
        </div>
      </StudentLayout>
    );
    ...
    return (
      <StudentLayout>
        <div className="max-w-md mx-auto bg-white rounded-[var(--radius-xl)] border border-[var(--color-border)] p-8 text-center space-y-4 shadow-xs">
          ...
          <h2 className="text-base font-bold text-[var(--color-text-primary)]">Evaluation Not Available</h2>
  ...
  // Extract Q&A pairs from messages for transcript review
      ...
      <div className="max-w-4xl mx-auto space-y-8 pb-16">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">
              Adaptive Interview Performance Report
            </h1>
            <p className="text-xs text-[var(--color-text-secondary)] mt-0.5 font-medium">
              {trackMeta.title} • {session.target_role || 'Software'} • {session.experience_level || 'Fresher'}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white rounded-[var(--radius-xl)] border border-[var(--color-border)] p-5 shadow-[var(--shadow-xs)] space-y-1">
            ...
            <p className="text-[11px] text-[var(--color-text-secondary)]">Baseline candidate selection</p>
          </div>
          ...
          <div className="bg-white rounded-[var(--radius-xl)] border border-[var(--color-border)] p-5 shadow-[var(--shadow-xs)] space-y-1">
            ...
            <p className="text-[11px] text-[var(--color-text-secondary)]">Highest adaptive difficulty</p>
          </div>

          <div className="bg-white rounded-[var(--radius-xl)] border border-[var(--color-border)] p-5 shadow-[var(--shadow-xs)] space-y-1">
            <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-tertiary)] flex items-center gap-1">
              <Activity className="w-3.5 h-3.5 text-blue-500" /> Final Momentum
            </span>
            <h3 className="text-lg font-extrabold text-[var(--color-text-primary)] capitalize">{momentum.replace('_', ' ')}</h3>
            <p className="text-[11px] text-[var(--color-text-secondary)]">Performance trajectory</p>
          </div>
        </div>
        ...
        {topicPerformance.length > 0 && (
          <div className="bg-white rounded-[var(--radius-xl)] border border-[var(--color-border)] p-6 shadow-[var(--shadow-xs)] space-y-4">
            <div>
              <h3 className="text-sm font-bold text-[var(--color-text-primary)]">Topic Performance Breakdown</h3>
              <p className="text-xs text-[var(--color-text-secondary)]">Topic strengths evaluated by the adaptive AI engine.</p>
            </div>
              ...
              {topicPerformance.map((tp, idx) => (
                <div key={idx} className="p-4 bg-[var(--color-bg-subtle)] rounded-[var(--radius-lg)] border border-[var(--color-border)] flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <span className="text-[11px] text-[var(--color-text-tertiary)]">{tp.attempts} attempt{tp.attempts > 1 ? 's' : ''}</span>
                  </div>
        ...
        {recommendationsList.length > 0 && (
          <div className="rounded-[var(--radius-xl)] border border-[var(--color-border)] bg-white p-6 shadow-[var(--shadow-xs)] space-y-4">
            <h3 className="text-sm font-bold text-[var(--color-text-primary)] flex items-center gap-2">
              <BookOpen className="w-4 h-4 text-[var(--color-brand-600)]" /> Next Step Practice Recommendations
            </h3>
                ...
                <div key={i} className="p-4 rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-subtle)] flex flex-col justify-between space-y-2">
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-tertiary)] block">
                      {rec.type === 'interview' ? 'Question Prep' : rec.type === 'notes' ? 'Revision Guide' : 'Skill Mastery'}
                    </span>
        ...
        {feedbackText && (
          <div className="rounded-[var(--radius-xl)] border border-[var(--color-border)] bg-white p-6 shadow-[var(--shadow-xs)] space-y-2">
            <h3 className="text-sm font-bold text-[var(--color-text-primary)] flex items-center gap-2">
              <MessageSquare className="w-4 h-4 text-[var(--color-brand-600)]" /> General Feedback & Next Steps
            </h3>
          ...
          <div className="rounded-[var(--radius-xl)] border border-[var(--color-border)] bg-white p-6 shadow-[var(--shadow-xs)] space-y-4">
            <div>
              <h3 className="text-base font-bold text-[var(--color-text-primary)]">
                Complete Interview Transcript ({qaPairs.length} Exchanges)
              </h3>
              <p className="text-xs text-[var(--color-text-secondary)]">
                Review all questions and responses from this practice session.
              </p>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4">
          ...
          <Link href="/student/mock-interview">
            <Button variant="primary" size="md" className="text-xs w-full sm:w-auto shadow-xs">
              <RotateCcw className="w-4 h-4 mr-1.5" /> Practice Another Track
            </Button>
          </Link>
        </div>
