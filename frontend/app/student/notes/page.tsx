    ...
    <StudentLayout>
      <div className="max-w-7xl mx-auto space-y-8 pb-12">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Access interview preparation notes, programming guides, aptitude material, and career resources.</p>
        </div>

        {/* SEARCH & FILTERS */}
        <Card className="p-4 border-gray-200 shadow-sm flex flex-col sm:flex-row gap-4 items-center">
        ...
        {!categoryFilter && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {CATEGORIES.map(cat => (
              <Card 
                key={cat.name} 
                className="p-4 border-gray-200 hover:border-blue-400 hover:shadow-md cursor-pointer transition-all flex flex-col items-center justify-center text-center gap-2 group"
        ...
        <div>
          {categoryFilter && (
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-sm text-gray-500">({filteredNotes.length} found)</span>
            </div>
          )}
          ...
          {isFetching ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredNotes.map(note => {
                ...
                return (
                  <Card key={note.id} className="border-gray-200 hover:border-blue-300 transition-all flex flex-col h-full bg-white">
                    {/* Card Top */}
                    <div className="p-5 flex-1 cursor-pointer" onClick={() => openNoteDetails(note)}>
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <button 
                          ...
                          className={`p-1.5 rounded-full transition-colors ${isSaved ? 'text-blue-600 bg-blue-50 hover:bg-blue-100' : 'text-gray-400 hover:bg-gray-100'}`}
                    ...
                    <div className="grid grid-cols-2 border-t border-gray-100">
                      <button 
                        ...
                        className="py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-blue-600 flex items-center justify-center gap-2 border-r border-gray-100 transition-colors"
                      >
                        <Eye className="w-4 h-4" /> View
                      </button>
                      <button 
                        onClick={() => handleDownload(note)}
                        className="py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-blue-600 flex items-center justify-center gap-2 transition-colors"
                      >
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Note Details" className="max-w-2xl">
        {selectedNote && (
          <div className="space-y-6">
            ...
            {selectedNote.tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {selectedNote.tags.map(tag => (
                  <span key={tag} className="text-xs font-medium text-gray-600 bg-gray-100 px-2 py-1 rounded">
                    #{tag}
                  </span>
                ))}
              </div>
            ...
            {/* File Info */}
            <div className="flex items-center gap-6 p-4 border border-gray-200 rounded-xl">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500">Size: {selectedNote.file_size}</p>
              </div>
            </div>
