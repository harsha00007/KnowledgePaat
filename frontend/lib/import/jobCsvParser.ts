...
/**
 * Parses CSV text content into structured raw job rows
 */
export function parseJobCSV(csvText: string, fileName: string, fileSizeBytes: number): JobParseResult {
  ...
  try {
    ...
    if (!parsed.data || parsed.data.length === 0) {
      return {
        ...
        rows: [],
        warnings,
        error: 'The uploaded CSV file is empty or could not be parsed.',
      };
    }
    ...
    parsed.data.forEach((row, index) => {
      const canonicalRow: RawJobRow = {
        rowNumber: index + 2, // 1-based, CSV row 1 is header
      };
    ...
    console.error('Error parsing Job CSV file:', err);
    return {
      fileName,
      fileType: 'csv',
      fileSizeBytes,
      ...
      rows: [],
      warnings: [],
      error: `Failed to parse CSV file: ${err.message || 'Invalid CSV syntax.'}`,
    };
  }
}
